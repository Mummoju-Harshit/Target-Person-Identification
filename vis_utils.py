...
def draw_bounding_box_on_image_array(image,
  ...
  #Adding cropped images folder
  if not os.path.exists('persons'):
    os.makedirs('persons')

  for i in range(len(bordered_images)):
    img = bordered_images[i]
    img_pil = Image.fromarray(img)
    img_pil.save(f'persons/person_{i}.jpg', 'JPEG')
...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    if "person" in display_str.lower():
        print(left,right,top,bottom)
        # Calculate the bounding box dimensions in integer pixels
        bbox = [int(left), int(top), int(right), int(bottom)]
        # Extract the sub-image using the bounding box dimensions
        bordered_image = copy_image.crop(bbox)
        # Add the bordered image to the list
        bordered_image = np.asarray(bordered_image)
